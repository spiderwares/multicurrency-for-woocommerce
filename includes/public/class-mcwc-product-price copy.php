...
if ( ! class_exists( 'MCWC_Product_Price' ) ) :
    ...
    class MCWC_Product_Price {
        ...
        private function event_handler(){
            ...
            add_filter( 'woocommerce_available_variation', array( $this, 'convert_ajax_variation_price' ), 20, 3 );
        ...
        public function convert_price_by_currency( $price, $product ) {
            // Don't alter or format if no price is set
        ...
        /**
         * Converts the variable price range HTML output.
         *
         * @param string        $price_html The raw price HTML.
         * @param WC_Product    $product    The product object.
         * @return string
         */
        public function convert_price_html_for_variable_product( $price_html, $product ) {
            if ( ! $product->is_type( 'variable' ) ) :
                return $price_html; // Only apply to variable products
            endif;

            $selected_currency = $this->mcwc_get_selected_currency();
            $currencies        = isset( $this->settings['currencies'] ) ? $this->settings['currencies'] : [];

            if ( empty( $selected_currency ) || empty( $currencies ) ) :
                return $price_html;
            endif;
            ...
            $fee            = isset( $currency_data['fee'] ) ? floatval( $currency_data['fee'] ) : 0;
            $decimals       = isset( $currency_data['decimals'] ) ? intval( $currency_data['decimals'] ) : 2;
            $symbol         = isset( $currency_data['symbol'] ) ? $currency_data['symbol'] : get_woocommerce_currency_symbol( $selected_currency );
            $converted_min  = ( $min_price * $rate ) + $fee;
            $converted_max  = ( $max_price * $rate ) + $fee;
            $formatted_min  = wc_price( round( $converted_min, $decimals ), [ 'currency' => $selected_currency ] );
            $formatted_max  = wc_price( round( $converted_max, $decimals ), [ 'currency' => $selected_currency ] );

            return $formatted_min . ' – ' . $formatted_max;
        }

        /**
         * Modify variation prices in AJAX response to apply currency conversion.
         *
         ...
         * @return array
         */
        public function convert_ajax_variation_price( $variation_data, $product, $parent ) {
            $selected_currency = $this->mcwc_get_selected_currency();
            $currencies        = isset( $this->settings['currencies'] ) ? $this->settings['currencies'] : [];
            ...
            $converted_regular  = ( $regular_price * $rate ) + $fee;
            $converted_sale     = ( $sale_price * $rate ) + $fee;

            $variation_data['display_price']            = $converted_sale > 0 ? round( $converted_sale, $decimals ) : round( $converted_regular, $decimals );
            $variation_data['display_regular_price']    = round( $converted_regular, $decimals );
            $variation_data['display_price_html']       = wc_price( $variation_data['display_price'], [ 'currency' => $selected_currency ] );
